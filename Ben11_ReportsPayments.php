        ...
        if (move_uploaded_file($_FILES['report_file']['tmp_name'], $target_file)) {
            ...
            if ($check_result->num_rows > 0) {
                $update_sql = "UPDATE academic_report
                               SET report_file = ?, report_upload = 'مرفوع', report_appoval = 'غير معتمد', submit_date = NOW()
                               WHERE payment_id = ? AND contract_id = ? AND bnf_id = ?";
...
$res_p = $conn->query("
    SELECT p.payment_id, p.installment_number, p.payment_status,
           r.report_upload, r.report_appoval, r.report_file
    FROM payments p
...
<head>
    ...
    <style>
        ...
        .styled-table td { padding: 18px 14px; text-align: center; border-bottom: 1px solid #F1F1F1; color: #444444; font-size: 14px; background: #FFFFFF; }
        ...
        .txt-gray { color: #888888; font-weight: 700; }
            ...
            <div class="content-grid">
                <div class="info-card contract-box">
                    ...
                    <div class="data-row">
                        ...
                        <span>CNT-<?php echo $info['contract_id']; ?></span>
                    </div>

                    <div class="data-row">
                        <label>قيمة المنحة الإجمالية:</label>
                        <span><?php echo number_format($info['amount']); ?> ريال</span>
                    </div>
                                            ...
                                            <?php } else { ?>
                                                <form method="POST" enctype="multipart/form-data" class="upload-form">
                                                    ...
                                                    <label class="btn-upload">
                                                        ارفع التقرير
<input type="file" name="report_file" accept=".pdf" onchange="this.form.submit()" style="display:none;">                                                    </label>
                                                </form>
                                            <?php } ?>
                                        </div>
                                    </td>
