...
/** التحقق من تسجيل دخول المستفيد قبل فتح صفحة التعديل **/
...
if (isset($_POST["save"])) {
    ...
    /** التحقق من صحة البيانات قبل تنفيذ التحديث **/
    if ($emailResult->num_rows > 0) {
        ...
        /** تنفيذ تعديل كلمة المرور فقط إذا قام المستفيد بتعبئة أحد حقولها **/
        if ($currentPass != "" || $newPass != "" || $confirmPass != "") {

            /* جلب كلمة المرور الحالية المشفرة من قاعدة البيانات */
            $getPass = $conn->prepare("
                SELECT password
                FROM beneficiary
                WHERE bnf_id = ?
            ");
            ...
            /** التأكد من اكتمال حقول كلمة المرور قبل تعديلها **/
            if ($currentPass == "" || $newPass == "" || $confirmPass == "") {
                $msg = "لتغيير كلمة المرور يجب تعبئة جميع حقول كلمة المرور.";
                $type = "error";

            /** التحقق من مطابقة كلمة المرور الحالية مع الكلمة المحفوظة **/
            } elseif (!password_verify($currentPass, $storedPass)) {
                $msg = "كلمة المرور الحالية غير صحيحة.";
                $type = "error";

            /** التأكد من تطابق كلمة المرور الجديدة مع تأكيدها **/
...
/* جلب بيانات المستفيد الحالية لعرضها داخل النموذج */
$stmt = $conn->prepare("
    SELECT
        f_name,
        l_name,
...
/** التأكد من وجود بيانات المستفيد قبل عرض النموذج **/
...
/* تحديد عرض صندوق النموذج */
.box{ width:760px; }

/* ملاحظة كلمة المرور */
.note{ font-size:12px; color:#666666; margin-top:5px; }

/* زر الإلغاء */
.cancel-btn{ background:#888888; margin-right:10px; }
</style>
</head>
    ...
    <div class="main-content">
        ...
        <div class="page">
            ...
            <div class="box">
                ...
                <!-- عرض رسالة نجاح أو خطأ بعد محاولة الحفظ -->
                <?php if ($msg != "") { ?>
                ...
                <form method="post">
                    ...
                    <div class="center">
                        ...
                        <button type="button" class="btn cancel-btn" onclick="window.location.href='Ben02_Profile.php'">
                            إلغاء
                        </button>
                    </div>
