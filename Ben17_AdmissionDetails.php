...
if (isset($_POST['send_rating'])) {
    ...
    if ($status == "" || $status == "قيد المعالجة") {
        ...
        $sqlInsertRating = "INSERT INTO rating (bnf_id, office_id, comment_text)
                            VALUES ('$bnf_id', '$office_id', '$safeComment')";

        if ($conn->query($sqlInsertRating)) {
            $msg = "تم إرسال التقييم بنجاح.";
            $type = "success";
        } else {
...
/* جلب آخر تقييم */
$lastComment = "";
$sqlLastRating = "SELECT comment_text
                  FROM rating
                  WHERE bnf_id = $bnf_id
...
.content-box{ width:100%; max-width:930px; margin:18px auto; background:#fff; border:1px solid #ece7f1; border-radius:16px;  overflow:hidden; }
.top-card{ padding:20px 26px 16px; background:#FCFBFD; border-bottom:1px solid #ECE7F1; }
.top-row{ display:flex; justify-content:space-between; align-items:flex-start; gap:16px; margin-bottom:14px; }
.badge{ min-width:205px; text-align:center; color:#fff; font-size:15px; font-weight:700; padding:10px 16px; border-radius:10px; }
.badge.processing{ background:#E5C06A; color:#5C4400; }
.badge.done{ background:#71BC93; }
.badge.rejected{ background:#E6A9A9; color:#7A1F1F; }
.top-title{ color:#2E1D45; font-size:17px; font-weight:700; text-align:right; }
.request-grid{ display:grid; grid-template-columns:repeat(3, 1fr); gap:16px; align-items:start; }
.request-item{ text-align:right; }
.request-label{ color:#8AAFC0; font-size:13px; font-weight:700; margin-bottom:4px; }
.request-value{ color:#444; font-size:14px; line-height:1.8; }
.section-box{ padding:20px 26px; border-bottom:1px solid #ECE7F1; }
.section-box:last-child{ border-bottom:none; }
.section-title{ color:#2E1D45; font-size:18px; font-weight:700; margin-bottom:10px; }
.section-note{ color:#7A7A7A; font-size:13px; margin-top:-4px; margin-bottom:12px; }
.result-box{ background:#EEF6F7; border:1px solid #D6E6E8; border-radius:12px; padding:16px 18px; line-height:2; color:#555; font-size:14px; }
.result-box.processing{ background:#FFF8E8; border:1px solid #F0D89A; }
.result-box.rejected{ background:#FFF1F1; border:1px solid #E8C1C1; }
.rating-form{ display:flex; flex-direction:column; gap:14px; }
.rating-textarea{ width:100%; min-height:110px; resize:none; border:1px solid #D7E5E8; border-radius:12px; background:#F4FBFC; padding:14px; font-size:14px; outline:none; color:#444; font-family:'Noto Kufi Arabic', sans-serif; }
.rating-textarea::placeholder{ color:#95AAB0; }
.rating-btn-wrap{ display:flex; justify-content:flex-end; }
.rating-btn{ min-width:170px; text-align:center; background:#4F9FB8; color:#fff; border:none; border-radius:12px; padding:12px 18px; font-size:15px; font-weight:700; cursor:pointer; font-family:'Noto Kufi Arabic', sans-serif; transition:0.3s; }
.rating-btn:hover{ background:#428AA0; }
.msg{ max-width:930px; margin:0 auto 14px; padding:12px; border-radius:8px; text-align:center; font-size:14px; }
.msg.success{ background:#EEF8F0; color:#1E6B35; border:1px solid #B9DFC2; }
.msg.error{ background:#FFF1F1; color:#B42318; border:1px solid #E8B4B4; }
.back-btn{ width:50px; height:50px; display:flex; align-items:center; justify-content:center; }
.back-wrap{ display:flex; justify-content:flex-end; max-width:930px; margin:0 auto 10px; }
        ...
        <?php echo $statusText; ?>
                ...
                <div class="section-box">
                    ...
                    <form method="POST" class="rating-form">
                        <textarea name="comment_text" class="rating-textarea" placeholder="اكتب تقييمك لخدمة المكتب هنا"><?php echo htmlspecialchars($lastComment); ?></textarea>
