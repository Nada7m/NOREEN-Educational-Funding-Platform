...
/** التحقق من دخول الأدمن قبل السماح بعرض الصفحة **/
...
/* حساب إجمالي المستفيدين المسجلين */
$beneficiaries_count = 0;

$q1 = mysqli_query($con, "
    SELECT COUNT(bnf_id) AS total
    FROM beneficiary
");

/** استخراج العدد الإجمالي للمستفيدين من نتيجة الاستعلام **/
if ($q1) {
    $row1 = mysqli_fetch_assoc($q1);
    $beneficiaries_count = $row1['total'];
}

/* حساب إجمالي المستثمرين المسجلين */
$investors_count = 0;

$q2 = mysqli_query($con, "
    SELECT COUNT(inv_id) AS total
    FROM investor
");

/** استخراج العدد الإجمالي للمستثمرين من نتيجة الاستعلام **/
if ($q2) {
    $row2 = mysqli_fetch_assoc($q2);
    $investors_count = $row2['total'];
}

/* حساب إجمالي المكاتب الاستشارية المسجلة */
$offices_count = 0;

$q3 = mysqli_query($con, "
    SELECT COUNT(office_id) AS total
    FROM consulting_office
");

/** استخراج العدد الإجمالي للمكاتب الاستشارية من نتيجة الاستعلام **/
if ($q3) {
...
/* الحفاظ على توزيع الكروت بشكل متساوٍ داخل الصفحة */
...
/* إبراز الرقم الإحصائي داخل الكرت */
