...
// إذا لم يسجل دخول يتم تحويله لصفحة الدخول
...
// جلب رقم المكتب الحالي من الجلسة
$current_off_id = $_SESSION['office_id'];
// جلب الاستشارات الخاصة بالمكتب
$sql = "SELECT 
            b.bnf_id, 
...
/* كرت الجدول */
.table-card{background:#FFFFFF;border:1px solid #E8E8E8;border-radius:18px;box-shadow:0 2px 8px rgba(0,0,0,0.04);
    overflow:hidden;}
/* الجدول */
.consult-table{width:100%;border-collapse:collapse;background:#FFFFFF;}
/* رأس الجدول */
.consult-table th{background:#FFFFFF;color:#3E2454;font-size:16px;font-weight:700;
    padding:18px 20px;text-align:center; border-bottom:1px solid #DCDCDC;}
/* خلايا الجدول */
.consult-table td{
    padding:18px 20px;text-align:center; font-size:14px; color:#555555;
...
/* التاريخ */
.send-date{color:#7A7A7A;}
/* زر الإجراء */
.btn-action{ background:#E5E7EB; color:#000000; padding:10px 22px; border-radius:10px; text-decoration:none;
    font-size:14px;border:1px solid #CCCCCC;display:inline-flex; align-items:center; justify-content:center;
    min-width:170px;font-weight:600;}
/* زر وجود رسالة جديدة */
.btn-green-alert{ background:#D1FAE5 !important; color:#065F46 !important; border:1px solid #10B981 !important;
    font-weight:700;}
/* صف عدم وجود بيانات */
                            ...
                            <?php if ($result && $result->num_rows > 0): ?>
                                <?php while($row = $result->fetch_assoc()): 
                                    ...
                                    $last_off = $row['last_off_time'];
                                      // التحقق هل توجد رسالة جديدة
                                    $is_new = (!$last_off || ($last_bnf > $last_off));
                                    // تغيير لون الزر إذا توجد رسالة جديدة
